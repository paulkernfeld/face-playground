...
test.describe("Loading diagnostics", () => {
  ...
  test("E — All experiments smoke test", async ({ page }) => {
    ...
    if (failed.length === 0) {
      test.info().annotations.push({
        type: "diagnosis",
        description: "All 10 experiments loaded successfully.",
      });
      ...
      test.info().annotations.push({
        type: "diagnosis",
        description: `${failed.length}/10 experiments failed: ${failed.map((r) => `exp=${r.exp}`).join(", ")}`,
      });
    }
