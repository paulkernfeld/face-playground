...
**Experiments** (1-indexed for `?demo=N`): 1=headCursor, 2=faceChomp, 3=bodyCreature, 4=redLightGreenLight, 5=ddr, 6=yoga, 7=posture, 8=mindfulness, 9=heartbeat, 10=kasina, 11=morphcast.
...
- **QA (V→done)**: For **(a)-(c)** (automated tests): run the tests — if they pass, merge and remove the row. For **(d)-(f)** (human-verified): tell the user **what to look for** first (e.g. "left arm should be charcoal, right arm sage green"), **then** show the screenshot or provide the link, and **wait for explicit user approval before merging**. Do not self-approve visual or interactive verification — the whole point of (d)-(f) is that a human judges it. If the user approves, merge the worktree branch into master, delete the worktree, and remove the TODO row. If not, move back to **I** with notes. Ralph loop end condition: no V-status items remain, all merged to master, worktrees cleaned up.
...
| Feature | St | Plan |
|---------|----|------|
| **DDR: confusing UX** — rhythm game is confusing, needs redesign | D | Needs design pass — what specifically is confusing? Timing? Visuals? Arrow meaning? Feedback? |
| **Mindfulness: too sensitive** — body movement detection makes it impossible to complete | D | Movement threshold too tight — user can't stay still enough. Needs tuning or forgiveness window. |
...
| **QA annotation overlay** — when taking screenshots or doing manual QA, draw labeled arrows/highlights on the screenshot showing what the user should look for | B | QA Claude generates an annotated version of each screenshot with callouts (e.g. "text should be charcoal here", "background should be cream", arrow pointing to HUD). Could be a canvas overlay, a post-processing script, or Playwright drawing annotations. Acceptance: QA screenshots include visual callouts that guide the reviewer's eye. QA: **(d)** user reviews annotated screenshot |
| **Mindfulness: more check types** — yoga poses, eyes closed, stretching | B | Diverse exercises beyond "stay still" — leverage existing experiments (yoga, blendshapes) |
...
- **No multi-line bash scripts** — keep each Bash tool call to a single simple command. Don't chain with `&&`, don't use `sleep`, don't combine background processes with waits. Multi-step commands (e.g. `kill ...; sleep 1; lsof ...`) require manual approval and block automation. If commands need to run sequentially, use separate Bash calls.
