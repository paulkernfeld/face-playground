...
| **DDR: detection feels laggy** — ~half-beat delay, you have to position your head early | D | Flipped offset didn't help — hard to tell what's wrong without debugging. Needs: (1) visual indicator showing what arrow direction the head is currently detected as, (2) visual indicator for when the hit check is happening, (3) consider scheduling hit/miss sound on the *following* beat since detection may need to happen after the current beat. Design the debugging UX first, then iterate on timing. |
