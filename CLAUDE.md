...
| Feature | St | Plan |
|---------|----|------|
| **Pixel-scale coordinates** — replace 16×9 game-units with pixel-scale coords; fixes fillText + rough.js | D | Use 1280x720 (or similar 16:9) as the fixed coordinate system instead of 16x9. All experiments, landmarks, and drawing use pixel-scale coords. GameRoughCanvas wrapper can be simplified/removed (no more 100x scaling hack). fillText works naturally. Need migration plan: update GAME_W/GAME_H constants, update all hardcoded coords in every experiment, update landmark scaling in main.ts. Big refactor — do one experiment at a time? |
| **Chomp: fruit bounces off you** when mouth closed | I | Behind a feature flag (URL param like `?chompBounce`). When mouth closed, fruit bounces off face instead of being eaten. When mouth open, normal chomp. Acceptance: with flag on, closing mouth makes fruit ricochet; without flag, behavior unchanged. QA: **(e)** [`?exp=2&chompBounce`](http://localhost:5199/?exp=2&chompBounce) — close mouth near fruit, see it bounce away |
| **UI: remove screenshot button** — remove 's' key and screenshot button from touch bar | I | Remove screenshot button from touch bar and 's' key handler. Reduces clutter. Acceptance: screenshot button gone from UI, 's' key does nothing. QA: **(b)** Playwright test: verify touch bar has only back + debug buttons, no screenshot |
| **Offline/PWA** — airplane mode on phone | I | Cache everything on first load: service worker precaches all JS/HTML/CSS + WASM models (~15MB). manifest.json for installability. Cache-first strategy (all client-side, no server calls). Acceptance: after first visit with internet, app works fully on airplane mode. QA: **(e)** load app, go offline (devtools Network→Offline), reload — all experiments should still work |
| **Light background** — dark→light canvas bg | I | Use warm cream (`cream` from palette, #F5F0E8) as canvas background instead of dark navy. Change `canvasBg` in palette.ts. Charcoal strokes still contrast well on cream. Acceptance: all experiments render on a warm cream background. QA: **(d)** `?demo=3` screenshot — creature should look good on cream bg |
| **DDR: detection feels laggy** — ~half-beat delay, you have to position your head early | I | Add debug overlay: (1) show current detected head direction as text/arrow in corner, (2) highlight the hit window timing on the arrow track (colored band showing when detection is active), (3) schedule hit/miss sound on following beat. Acceptance: user can see what direction is detected and when the check happens, making timing issues diagnosable. QA: **(e)** [`?exp=5`](http://localhost:5199/?exp=5) — debug overlay visible, head direction updates in real-time, hit window band visible on arrow track |
| **DDR: camera angle calibration** — baseline neutral pitch | I | On DDR start, show "look straight ahead" for 3s, record average pitch/yaw as baseline, subtract from all subsequent detections. Acceptance: user with tilted camera/head can play DDR normally after calibration. QA: **(e)** [`?exp=5`](http://localhost:5199/?exp=5) — tilt head slightly before starting, calibration should compensate |
| **Mindfulness: audio feedback** — sound cues for phase transitions (start, interrupt, complete) | I | Synth tones via Web Audio (reuse playDing pattern): gentle sine sweep on start, soft damped alert on interrupt, bright ding on complete. Acceptance: user hears distinct sounds for each phase transition. QA: **(e)** [`?exp=8`](http://localhost:5199/?exp=8) — close eyes (hear start tone), move (hear interrupt tone), complete 10s (hear success ding) |
| **Mindfulness: lock done** — prevent accidental restart after completing session | I | After 10s success, show "well done" permanently. Opening eyes shows success screen (no restart). Must press key/tap "restart" button to begin new session. Acceptance: completing session locks to success screen until deliberate restart. QA: **(e)** [`?exp=8`](http://localhost:5199/?exp=8) — complete 10s, open eyes, verify no restart. Press key to restart. |
| **Warning system refactor** — main.ts warnings vs experiment warnings overlap | D | User idea: use the existing update() callback that main.ts calls — have it return warning text, and main.ts renders it. Need to check: update() currently returns void. Could add a return type, or add a separate getWarning() method to the Experiment interface. Design the exact API change. |
| **Research open source related work** — face-tracking games, body-tracking art, WebRTC experiments | I | Web-search for open source face/body tracking projects, interactive art, WebRTC experiments. Add interesting findings + links to CLAUDE.md experiment ideas section. Acceptance: at least 5 relevant projects documented. QA: **(a)** user reviews additions to experiment ideas section |
| **Research commercial competitors** — existing face/body tracking apps and games | I | Web-search for commercial face/body tracking apps, games, and experiences. Add findings to CLAUDE.md. Acceptance: at least 5 relevant products documented. QA: **(a)** user reviews additions |
...
| **Quick capture mode** — `?capture` currently requires timer; add instant-capture option | I | In `?capture` mode, pressing Space captures the current frame immediately (no countdown). Toggle video with `v` as before. Acceptance: user presses Space, frame saved instantly to `fixtures/<name>.png`. QA: **(b)** Playwright test: navigate to `?capture&prompt=test`, press Space, verify file saved |
| **Privacy containment** — prevent Claude from seeing room contents via camera→screenshot pipeline | D | Direction: never screenshot the video layer, only the canvas overlay. Needs more fleshing out: how to enforce this (hook? code constraint?), what about `?capture` mode which intentionally captures video, edge cases. |
